...
import { FaTimes, FaStar, FaPlane, FaThumbsUp } from 'react-icons/fa';
...
/**
 * FlightReviewsModal — lists all passenger reviews for a flight,
 * shows aggregate stats, and lets logged-in users submit new reviews.
 */
const FlightReviewsModal = ({ isOpen, onClose, flight }) => {
  ...
  const flightName = flight
    ? `${flight.airline} ${flight.flightId} (${flight.departureAirport?.code} → ${flight.arrivalAirport?.code})`
    : '';
  ...
  const fetchReviews = async () => {
    ...
    try {
      const [revRes, statRes] = await Promise.all([
        axios.get(`${API_BASE}/api/reviews/flight/${flightId}`),
        axios.get(`${API_BASE}/api/reviews/flight/${flightId}/stats`),
      ]);
            ...
            >
              ...
              <div className="flex items-center justify-between p-5 border-b shrink-0">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h2 className="font-bold text-gray-800">Passenger Reviews</h2>
                    <p className="text-xs text-gray-500">{flightName}</p>
                  </div>
                </div>
